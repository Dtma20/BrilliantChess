...
def test_returns_no_sacrifice_when_no_piece_is_offered():
    ...
    assert detect_destination_offer(board, position, "g1f3") == NO_SACRIFICE
...
def test_rejects_a_capturable_pawn_offer():
    ...
    assert detect_destination_offer(board, position, "e3e4") == NO_SACRIFICE


def test_detects_a_capturable_non_pawn_offer():
    board = PythonChessBoardService()
    position = board.position_after("7r/7p/8/7Q/8/8/8/6KR w - - 0 1", ())

    evidence = detect_destination_offer(board, position, "h5h7")
